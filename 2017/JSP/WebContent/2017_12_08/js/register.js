...
$(document).ready(function(){
	$("#id_check").click(function(){
		if($.trim($("#id").val())){
			...
			$.ajax({
				type:"POST",
				url:"/study/IdCheck",//학원 Hyundai2  집 study
				data:jsonv,
				success:function(data){//data는 리턴값 변수 서블릿의의 결과를 받는다.					
					if(data == 1){
						$('#id_li span').remove();//혹시 기존에 실패했을 경우 기존 구문을 삭제
						$('#id_li').append("<span>사용하실 수 있는 아이디 입니다.</span>")
						//성공메세지를 생성해준다.
						$('#id_li span').css('color', 'green');//성공했으므로 초록색으로
					}else if(data == -1){
						$('#id_li span').remove();
						$('#id_li').append("<span>이미 있는 아이디 입니다.</span>")
						$('#id_li span').css('color', 'red');
						$('#id').val("");
						$('#id').focus();
						check_stat = false;
					}//end else-------------------->
			...
			$('#id_li span').remove();
			$('#id_li').append("<span>아이디를 입력하세요.</span>")
			$('#id_li span').css('color', 'red');
			$('#id').focus();
			check_stat = false;
		}
	...
	$("#insertmember").click(function(){
		...
		if(status){
			...
			$.ajax({
				type:"POST",
				url:"/study/InsertMember",//학원 Hyundai2  집 study
				data:jsonv,
				success:function(data){//data는 리턴값 변수 서블릿의의 결과를 받는다.	
					if(data == 1){
